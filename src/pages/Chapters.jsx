...
export default function Chapters() {
  ...
  useEffect(() => {
    let mounted = true
    async function run() {
      try { const res = await api.chapters(id); if (mounted) setList(res.items || res || []) }
      ...
      <ul className="space-y-2">
        {list?.map((ch, i) => (
          <li key={ch.id || i} className="bg-white rounded-md border border-stone-200 px-4 py-3 flex items-center justify-between">
            ...
            <Link to={`/read/${encodeURIComponent(ch.id || ch.slug || ch.urlId || '')}`} className="px-3 py-1 rounded bg-brand-500 text-white">Read</Link>
          </li>
        ))}
      </ul>
    </section>
