        ...
        <div className="relative overflow-hidden rounded-2xl ring-1 ring-white/10 dark:ring-gray-700/30 min-h-[440px] md:min-h-[560px]"
          ...
          {activeItem && (
            <div className="relative z-10 p-4 sm:p-6 md:p-10">
              {/* Mobile Layout */}
              <div className="lg:hidden grid grid-cols-[96px_1fr] sm:grid-cols-[128px_1fr] gap-3 sm:gap-4 mt-[70%] sm:mt-56 items-start">
                {/* Cover Image - Left */}
                <div className="w-24 sm:w-32">
                  <div className="aspect-[3/4] rounded-lg overflow-hidden shadow-2xl">
                    <img 
                      src={getImage(pickImage(activeItem) || pickImage(info))} 
                      alt={activeItem?.title || info?.title || 'thumbnail'}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {/* Genre chips from endpoint */}
                  <div className="flex flex-wrap gap-2 mb-3">
                    {(() => {
                      const tagsRaw = info?.genres || info?.otherInfo?.tags || activeItem?.genres || []
                      const tags = Array.isArray(tagsRaw) ? tagsRaw : []
                      const label = (v) => typeof v === 'string' ? v.replace(/_/g,' ') : (v?.name || v?.tag || '')
                      const finalTags = tags.map(label).filter(Boolean)
                      return (finalTags.length ? finalTags : ['Unknown']).slice(0,4).map((t) => (
                        <span key={t} className="px-2 py-0.5 rounded bg-white/20 text-white text-[10px] font-medium">{t}</span>
                      ))
                    })()}
                  </div>
