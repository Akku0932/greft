...
const app = express();
const PORT = process.env.PORT || 2942; // default to your current exposed port
const BASE_URL = process.env.BASE_URL || 'http://key-switching.gl.at.ply.gg:2942';
...
app.get('/api/mp', async (req, res) => {
  try {
    ...
    res.status(502).json({ error: 'MP proxy failed', detail: error.message });
  }
});
...
app.get('/api/gf', async (req, res) => {
  try {
    ...
    if (!upstreamPath) return res.status(400).json({ error: 'Missing required query param p' });

    const ORIGINS = ['http://ger.visionhost.cloud:2056', 'https://ger.visionhost.cloud:2056'];
    ...
    for (const base of ORIGINS) {
      try {
        ...
        const body = await response.buffer();
        const contentType = response.headers.get('content-type') || 'application/json';
        res.set({ 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Methods': 'GET, OPTIONS', 'Content-Type': contentType });
        return res.send(body);
      } catch (err) { lastError = String(err); continue; }
    }
    throw new Error(lastError || 'GF upstream failed');
  } catch (error) {
    res.status(502).json({ error: 'GF proxy failed', detail: error.message });
  }
});
...
app.listen(PORT, () => {
  ...
  console.log(`API health check: ${BASE_URL}/api/mp?p=popular-updates`);
});
